...
class IOR(BrandsLog):
    ...
    def xaxis(self,side:str,thresholds:float) -> Dict[str,ByteString] :
        ...
        if side in "left":
            ...
            left_predict_img = self.inference(left_img,thresholds)
            img[:,:middle_point] = left_predict_img # image concatenated happend
        
        elif side in 'right':
            right_img = img[:,middle_point:]
            right_predict_img = self.inference(right_img,thresholds)
    ...
    def yaxis(self,side:str ,thresholds:float) -> Dict[str,ByteString] :
        ...
        if side in "top":
            ...
            top_predict_img = self.inference(top_img,thresholds)
            img[:middle_point] = top_predict_img
        
        elif side in "buttom":
            buttom_img = img[middle_point:]
            buttom_predict_img = self.inference(buttom_img,thresholds)
